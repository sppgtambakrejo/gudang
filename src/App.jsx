...
// Storage layer: dulu memakai window.storage bawaan Claude Artifacts.
// Sekarang diganti ke Supabase, memakai satu tabel key-value (kv_store)
// supaya seluruh logika aplikasi di atas (yang memanggil storageGet/storageSet
// per KEYS.users, KEYS.masuk, dst) tidak perlu diubah sama sekali.
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [u, m, k, s, mi, lg, pr, sp] = await Promise.all([
  ...
  const ctx = { session, stock, masuk, keluar, masterItems, saldoByPeriode, users, log, persist, addLog, periode, computeStockFor, suppliers, ensureSupplier };
...
function MasterBarang({ ctx }) {
  ...
  async function addItem(e) {
    ...
    let next;
    if (editKode) {
      next = masterItems.map((i) => (i.kode === editKode ? { kode: form.kode, nama: form.nama, satuan: form.satuan } : i));
    } else {
      next = [...masterItems, { kode: form.kode, nama: form.nama, satuan: form.satuan }];
    }
    await persist.master(next);
    if (form.saldo || form.harga) {
      await persist.saldoByPeriode({ ...saldoByPeriode, [periode.activeId]: { ...saldoAwal, [form.kode]: { saldo: Number(form.saldo) || 0, harga: Number(form.harga) || 0 } } });
    }
      ...
      <div className="grid-eq2">
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <div style={{ fontSize: 11.5, color: "#8B8371", lineHeight: 1.5, padding: "0 4px" }}>
            Catatan keamanan: aplikasi ini menyimpan data di penyimpanan artifact (dapat diakses semua pengguna artifact ini). Untuk penggunaan produksi yang aman, kredensial sebaiknya dipindahkan ke backend dengan password ter-hash.
          </div>
        </div>
      </div>
    </div>
